...
function attachCanvas() {
    ...
    canvas2btrans = document.getElementById('front2trans');

    setCanvas(document.getElementById('back'));
    canvas3 = document.getElementById('back');
    setCanvas(document.getElementById('backtrans'));
    canvas3trans = document.getElementById('backtrans');

    // set context
    ctx1 = document.getElementById('text').getContext('2d');
    ctx1trans = document.getElementById('texttrans').getContext('2d');
    ctx2 = document.getElementById('front').getContext('2d');
    ctx2trans = document.getElementById('fronttrans').getContext('2d');
    ctx2b = document.getElementById('front2').getContext('2d');
    ctx2btrans = document.getElementById('front2trans').getContext('2d');
    ctx3 = document.getElementById('back').getContext('2d');
    ctx3trans = document.getElementById('backtrans').getContext('2d');
...
function videoLoaded() {
    ...
    if (videosLoaded == videosToLoad) loadedText("videos", "Videos loaded");
}
...
function cacheVIDs() {
    videosToLoad = Object.keys(VIDs).length;

    if (videosToLoad > 0) {
        if (CONFIG.debugLogs) console.groupCollapsed("videos found");
        loadingLine("Loading videos", "videos");
    }
...
function updateDialog(str) {
    ...
    if (lookForText.test(str) == true) {
        textColour = str.match(lookForText)[1];
        let xpos = str.match(lookForText)[2];
        textXpos = parseInt(xpos, 10);
        let ypos = str.match(lookForText)[3];
        textYpos = parseInt(ypos, 10);
        let endpos = str.match(lookForText)[4];
        textEndpos = parseInt(endpos, 10);
    }
    ...
    if (lookForPos.test(str) == true) {
        ...
        if (textXpos == "RESET") {
            ...
            textXpos = parseInt(textXpos);
        } 
        
        if (textYpos == "RESET") {
            textYpos = DEFAULT.textYpos;
        } else{
            textYpos = parseInt(textYpos);
        } 
        
        if (textEndpos == "RESET") {
            textEndpos = DEFAULT.textEndpos;
        } else{
            textEndpos = parseInt(textEndpos);
        }

        if (textXpos == "RAND") textXpos = parseInt(Math.floor(Math.random() * CONFIG.resolutionX));
        if (textYpos == "RAND") textYpos = parseInt(Math.floor(Math.random() * CONFIG.resolutionY));
    };
    ...
    if (lookForPosY.test(str) == true) {
        textYpos = str.match(lookForPosX)[1];
    ...
    if (lookForLine.test(str) == true) {
        textLineheight =  str.match(lookForLine)[1];
        if (textLineheight == "RESET") {
            textLineheight = DEFAULT.textLineheight;
        } else{
            textLineheight = parseInt(textLineheight);
        } 
    };
    ...
    if (lookForSFXVol.test(str) == true) {
        ...
        if (gameplayVolumeSFX < 0) {
            console.warn("SFXVOL warning: Volume cannot be lower than 0")
            gameplayVolumeSFX = 0;
        }
        if (gameplayVolumeSFX > 1) {
            console.warn("SFXVOL warning: Volume cannot be higher than 1")
            gameplayVolumeSFX = 1;
        }
...
function drawDialogue(str, speaker) {
    ...
    if (lookForCon.test(str) == true) {
        ...
        // float checks because it breaks otherwise for SOME FUCKING REASON
        if (parseFloat(var1)) {
    ...
    if (speaker != "narration") {
        let spkrxpos = parseInt(textXpos - DEFAULT.speakerLeftOffset);
        let spkrendpos = parseInt(spkrxpos + DEFAULT.speakerEndpos);
        drawText(speaker.name, speaker.colour, textSize, textFont, spkrxpos, textYpos, spkrendpos, textLineheight);
    };
